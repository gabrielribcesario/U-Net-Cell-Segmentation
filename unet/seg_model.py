...
from tensorflow.keras.layers import (Conv2D, MaxPooling2D,  UpSampling2D, BatchNormalization, Activation, Concatenate, SpatialDropout2D)
...
class UNet(tf.keras.layers.Layer):
    def __init__(self, output_maps, output_act='softmax', activation='relu', 
                 kernel_initializer='HeNormal', bias_initializer='zeros', dropout=None,
                 kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, 
                 **kwargs):
        param = locals()
        param['padding'] = 'same'
        for i in ('__class__', 'output_maps', 'self', 'kwargs', 'output_act', 'dropout'): 
            param.pop(i)
        super().__init__()
